...
export default function ProfilePage() {
  ...
  const fetchUserAddresses = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/main/address'); // GET /api/main/address (collection)
      if (!response.ok) {
        const errorData = await response.json();
  ...
  const handleSaveAddress = async (e: React.FormEvent) => {
    ...
    showAlert('คุณแน่ใจหรือไม่ที่จะบันทึกที่อยู่นี้?', 'info', 'ยืนยันการบันทึกที่อยู่', async () => {
      try {
        ...
        let response;
        if (editingAddress) {
          // Update existing address: Call PUT /api/main/address/[addressId]
          response = await fetch(`/api/main/address/${editingAddress.Address_ID}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
          });
        } else {
          // Add new address: Call POST /api/main/address
          response = await fetch('/api/main/address', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
          });
  ...
  const handleDeleteAddress = async (addressId: number) => {
    showAlert('คุณแน่ใจหรือไม่ที่จะลบที่อยู่นี้?', 'warning', 'ยืนยันการลบที่อยู่', async () => {
      try {
        // Delete address: Call DELETE /api/main/address/[addressId]
  ...
  const handleSetDefaultAddress = async (addressId: number) => {
    ...
    showAlert('คุณแน่ใจหรือไม่ที่จะตั้งค่าที่อยู่นี้เป็นค่าเริ่มต้น?', 'info', 'ยืนยันที่อยู่เริ่มต้น', async () => {
      try {
        // Set default address: Call PUT /api/main/address/[addressId] to update Is_Default
        // The backend /api/main/address/[addressId] PUT handler will handle unsetting other defaults.
        const response = await fetch(`/api/main/address/${addressId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
