...
export default function RefundingPage() {
  ...
  const fetchOrderData = useCallback(async () => {
    ...
    async function loadNextStep(orderId: number) {
      const response = await fetch(`/api/admin/order/refund-step?controller=refunding&orderId=${orderId}`);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="max-w-5xl mx-auto p-6 space-y-8">
          <div className="bg-base-100 rounded-lg shadow-sm p-6 mb-6">
            <StepFlowBar currentStep={1} orderId={order.Order_ID} refund={true}/>
          </div>
